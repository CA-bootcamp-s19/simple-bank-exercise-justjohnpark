...
contract SimpleBank {
    ...
    // does amount need to be indexed?
    ...
    // do withdrawAmount & newBalance need to be indexed?
    ...
    function deposit() public payable returns (uint) {
        ...
        if (enrolled[user] == true) {
            uint previousBalance = this.getBalance();
            balances[user] = previousBalance + msg.value;
    ...
    function withdraw(uint withdrawAmount) public returns (uint) {
        ...
        address user = msg.sender;
        address payable u = msg.sender;

        if (balance >= withdrawAmount) {
            uint remaining = balance - withdrawAmount;
            u.transfer(withdrawAmount);
            balances[user] = remaining;
            emit LogWithdrawal(user, withdrawAmount, remaining);
            return remaining;
        } else {
            revert();
        }
